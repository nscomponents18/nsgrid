<div>
	...
	var arrItems = [];
	var column = [
					{headerText:"",dataField:"isEdit",width:"10%",draggable:false,sortable:false,sortDescending:false,itemRenderer:"editItemRenderer",
		      		...
		      		priority:1,enableEditing: false},
		      		{headerText:"Country",dataField:"country",width:"15%",sortable:true,sortDescending:true,draggable:false,resizable:true,showMenu:true,
		      			priority:2,enableEditing: false},
		      		{headerText:"Hierarchy",dataField:"hierarchy",width:"20%",sortable:true,sortDescending:false,showMenu:true,headerTruncateToFit:true,
		      			truncateToFit:true,priority:1},
		      		{headerText:"Year",dataField:"year",width:"20%",sortable:true,sortDescending:true,showMenu:true,priority:3,enableEditing: false},
		      		{headerText:"Employees",dataField:"employeesID",width:"30%",sortable:false,sortDescending:true,priority:4,
		      			enableFilter:false,enableEditable:false,enableEditing: false},
		      		{headerText:"City",dataField:"city",width:"20%",sortable:true,sortDescending:true,showMenu:true,truncateToFit:true,priority:5,enableEditing: false},
		      		{headerText:"Address",dataField:"address",width:"30%",sortable:true,sortDescending:true,showMenu:true,truncateToFit:true,priority:5,
		      			enableEditing: false},
		      		{headerText:"Date",dataField:"date",width:"70%",sortable:true,sortDescending:true,showMenu:true,labelFunction:"dateLabelFunction",
		      				truncateToFit:true,priority:5,enableEditing: false},
		      		
		      	];
	var setting = {nsTitle:"Editable Grid Demo With Initiator",enableEditable:true,
					...
					editorSetting:{clickType:NSGrid.EDITOR_EDITING_NOCLICK,enableMultipleEdit:true}};
	
	window["loadEditableGridWithInitiator"] = function ()
	{
		arrItems = [];
		var item = {};
		var count = 0;
	   ...
	   if(item)
       {
           ...
           if(typeof item.isEdit == "undefined" || !item.isEdit)
           {
               ...
               i.addEventListener("click",function(event)
                       {
                           item.isEdit = true;
                           nsGrid.updateItemInDataSource(item);
                           nsGrid.updateCellByKeyField(item["id"],dataField);
                           nsGrid.editByItem(item,"hierarchy");
                       });
               span.appendChild(i);
           }
           else
           {
               ...
               iCancel.addEventListener("click",function(event)
                   {
                       item.isEdit = false;
                       nsGrid.updateItemInDataSource(item);
                       nsGrid.updateCellByKeyField(item["id"],dataField);
                       nsGrid.editStopByItem(item,"hierarchy",true);
                   });
               ...
               iSave.addEventListener("click",function(event)
                   {
            	   		item.isEdit = false;
                   		nsGrid.updateItemInDataSource(item);
                   	 	nsGrid.updateCellByKeyField(item["id"],dataField);
                   		nsGrid.editStopByItem(item,"hierarchy",false);
                   });
	...
	//# sourceURL=editableGridWithInitiator.js
	</script>
</div>